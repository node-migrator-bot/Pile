    ...
    var Build = {
        __processLib: function(lib)
        {
            var name        = lib.name || (function(libUrl){
                ...
                override = lib.override || false;
            
            return {name: name, destination: destination, relative: lib.destination, url: lib.url, override: override}
        },
        ...
        generate: function(libs, dest)
        {
            ...
            var Handlebars  = require('Handlebars'),
                outputFile = "%@/%@".$$(process.cwd(), dest),
                jsFiles     = new Array(),
                cssFiles    = new Array(),
                source      = fs.readFileSync('%@/index.html'.$$(__dirname), 'utf8'),
            ...
            for(var i = 0; i < libs.length; i++)
            {
                ...
                if(lib.name.indexOf('js') > -1)
                    jsFiles.push(destination);
                    
                if(lib.name.indexOf('css') > -1)
                    cssFiles.push(destination)
            }
            
            fs.writeFile(outputFile, template({javascripts: jsFiles, css: cssFiles}), function(err){
                if(err)
                {
                    console.log(err);
                    process.exit(-1);
        ...
        download: function(libs)
        {
            for(var i = 0; i < libs.length; i++)
            {
                ...
                if(lib.override || !path.existsSync(lib.destination))
